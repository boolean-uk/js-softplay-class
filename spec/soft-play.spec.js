...
describe('Soft Play', () => {
  ...
  it('8 adults entering with 4 childlen- 1 adults leaving with 1 child', function () {
    expect(sp.enter(4, 2)).toBeTrue()
    expect(sp.enter(4, 2)).toBeTrue()
    expect(sp.leave(1, 1)).toBeTrue()
    expect(sp.total()).toEqual({ adults: 8, children: 4 })
  })
  it('8 adults entering with 4 childlen- 1 adults leaving with 1 child', function () {
    expect(sp.enter(4, 2)).toBeTrue()
    expect(sp.enter(4, 2)).toBeTrue()
    expect(sp.leave(1, 1)).toBeTrue()
    expect(sp.occupancy()).toEqual({ adults: 7, children: 3 })
  })
})
