...
describe('Soft Play', () => {
  ...
  it('Adults and children entering should be added to total values', function () {
    sp.enter(2, 2)
    expect(sp.enter(1, 1)).toBeTrue()
    expect(sp.occupancy()).toEqual({ adults: 3, children: 3 })
    expect(sp.total()).toEqual({ adults: 3, children: 3 })
  })

  it('Adults and children leaving should be not change the total values', function () {
    sp.enter(3, 3)
    expect(sp.leave(2, 2)).toBeTrue()
    expect(sp.occupancy()).toEqual({ adults: 1, children: 1 })
    expect(sp.total()).toEqual({ adults: 3, children: 3 })
